...
2. 번들 스크립트는 저장소에 **실행 권한 없이(`100644`) 추적**됩니다. `install.sh` 가 내부에서 `./prepare` 를 호출하므로 설치 전에 `chmod +x install.sh prepare` 가 필요합니다 — 없으면 `[Step 3]` 에서 `./prepare: Permission denied`(exit 126)로 중단됩니다.
3. `update_harbor_config.sh` 가 도메인·http 포트·https 여부를 입력받아 `harbor.yml` 을 만들고, `install.sh` 로 설치합니다. `autoinstall.sh` 는 두 단계를 한 번에 실행합니다. https 를 쓰려면 설치 전에 `compose/project_mng_service/harbor-v2.0.0/ssl/` 에 인증서를 둡니다. `install.sh` 재실행 시 기존 harbor 컨테이너를 `down -v` 로 내린 뒤 다시 올립니다.
